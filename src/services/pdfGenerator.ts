...
export const generateTechnicalReportPdf = async (projectInfo: ProjectInfo, title: string, content: string) => {
    try {
        const { jsPDF } = await getLibs();
        const doc = new jsPDF();
        const margin = 20;
        ...
        doc.setFontSize(22);
        doc.setFont("helvetica", "bold");
        doc.text("RELAZIONE TECNICA AI", pageWidth / 2, 20, { align: 'center' });
        doc.setFontSize(10);
        doc.setFont("helvetica", "normal");
        ...
        doc.setFontSize(9);
        doc.setFont("helvetica", "bold");
        doc.text("PROGETTO:", margin + 5, 58);
        doc.setFont("helvetica", "normal");
        doc.text(projectInfo.title, margin + 35, 58);
        
        doc.setFont("helvetica", "bold");
        doc.text("COMMITTENTE:", margin + 5, 65);
        doc.setFont("helvetica", "normal");
        doc.text(projectInfo.client, margin + 35, 65);

        doc.setFont("helvetica", "bold");
        doc.text("DATA REPORT:", margin + 5, 72);
        doc.setFont("helvetica", "normal");
        doc.text(new Date().toLocaleDateString('it-IT'), margin + 35, 72);
        ...
        drawSignature(doc, projectInfo, Math.min(currentY + 20, pageHeight - 40));

        window.open(URL.createObjectURL(doc.output('blob')), '_blank');
    } catch (e) {
        console.error("Technical Report PDF Error:", e);
    }
};
